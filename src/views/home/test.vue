<template>
  <div class="m-content">
      ...
      <div class="productlist">
        ...
        <div class="f-top">
          ...
          <div class="m-parts">
            ...
            <div class="f-right">
              <!-- 系统列表列表 -->
              <!-- <ul>
                <li class="secline" v-for="(item, index) in data.sysTypeList" :class="{'cf_bo': state.sysTypeIndex === index}" @click="selectType(item, 'sys', index)">{{item.name}}</li>
              </ul> -->

              <!-- 子类系统列表 -->
              <!-- <ul class="lastchild">
                <li v-for="(item, index) in data.subTypeList" :class="{'co': state.subTypeIndex === index}" @click="selectType(item, 'sub', index)">{{item.name}}</li>
              </ul> -->
            </div>
          </div>
          ...
          <div class="m-brand">
            <ul>
              ...
              <li>
                <div class="f-right">
                  <!-- <ul>
                    <li v-for="(item, index) in data.brandList" :class="{'co': state.brandIndex === index}" @click="selectType(item, 'brand', index)">{{item.name}}</li>
                  </ul> -->
                </div>
              </li>
            </ul>
        ...
        <div class="f-bottom">
          ...
          <div class="tit">
            ...
            <span class="result">找到约{{data.goods.totalCount}}条匹配结果</span>
          </div>

          <!-- 商品列表 -->
          <div class="productdetail">
            <!-- <ul class="pro-list" v-for="(item, index) in data.goods.list"> -->
              <!-- 商品图片 -->
    ...
    <div class="brandmask" v-show="showBrandmask">
      ...
      <h4>请选择您要找的汽车车型</h4>
      <ul class="nav">
        <li @click="showTab=1">1.选择品牌</li>
        <li @click="showTab=2">2.选择车系</li>
        <li @click="showTab=3">3.选择年份</li>
        <li @click="showTab=4">4.选择车型</li>
      </ul>
      ...
      <div class="select-list select-list1" v-show="showTab==1">
        <ul>
          <li v-for="(brand, index) in data.brandShortCut" @click="selectShortCut(shortCut, index)" :key="brand.ShortCutId" :class="{'active': shortCut == brand.ShortCutId}">
            {{brand.ShortCutId}}
          </li>
        </ul>
        <ul class="brand-list">
          <li v-for="(shortCutbrand, index) in data.shortCutbrandList" @click="selectBrand(shortCut, index)" :key="shortCutbrand.brandId" :class="{'active': index+1 == shortCutbrand.brandId}">
            {{shortCutbrand.name}}
          </li> 
        </ul>
      </div>
...
export default {
    ...
    methods: {
        ...
        getSubTypeList(id) {
            subtypeList({
                ...
                if (res.code === 0) {
                    ...
                    // 默认选择第一个子类系统ID
                    this.form.subtypeId = res.data[0].subtypeId;
        ...
        // 获取品牌列表
        getBrand(type) {
            salesBrandHot().then(res => {
                if (res.code === 0) {
                    res.data.unshift({
                        name: "全部",
                        salesBrandId: 0
                    });
                    this.data.brandList = res.data;
                }
            });
        },
        ...
        // 按缩写获取品牌数据
        // selectShortCut (brand, shortCut, index) {
        //    brandShortcut({
        //         shortCut: shortCut
        //     }).then(res => {
        //         if (res.code === 0) {
        ...
        .f-bottom {
            ...
            .tit {
                ...
                .price {
                    ...
                    .arrow-top {
                        background: url(img/p-arrow-top.png);
                    }
                    .arrow-bottom {
                        background: url(img/p-arrow-bottom.png);
                    }
                }
            }
            .productdetail {
